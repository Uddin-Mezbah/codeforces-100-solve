 ...
 /*Confident enough that i will get it all back*/
#include<bits/stdc++.h>
using namespace std;
...
// template <class T> void _print(T v[]) {cerr << "[ "; for (int i=0;i<sizeof(v);i++) {_print(v[i]); cerr << " ";} cerr << "]";}
...
vector<ll> getfactors(ll n)/*unsorted*/{vector<ll> factors;for(int i=1;i<=sqrt(n);i++){if(n%i==0){factors.push_back(i);if(n/i!=i)factors.push_back(n/i);    }}return factors;}
long long maxsubarraysum(vl array,long long n){long long best = 0, sum = 0;for (long long  k = 0; k < n; k++) {sum = max(array[k],sum+array[k]);best = max(best,sum);}return  best ;}
